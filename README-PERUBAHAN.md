...
## Revisi terbaru (putaran 21) - perbaikan ekstraksi

**TIDAK perlu migrasi SQL.** Tidak ada perubahan skema di putaran ini.

Diuji terhadap tiga PDF baru (satu FCL, satu sudah terdaftar, satu udara
dua barang). Dua miss ditemukan dan diperbaiki:

### Nama kapal satu kata selalu terbuang
Pencarian nama sarana pengangkut mensyaratkan barisnya MENGANDUNG SPASI -
penyaring supaya kata nyasar tidak ikut terambil. Efek sampingnya: nama
kapal satu kata selalu ditolak, dan itu umum sekali (`BELAWAN`, `MERATUS`,
`TIANJIN`). Sekarang patokannya POSISI, bukan bentuk nama: baris tepat di
bawah baris moda (`LAUT 1` / `UDARA 4`), melewati kode bendera dua huruf
dan berhenti di tanggal. Cara lama tetap dipakai sebagai cadangan.

### Total Package memuat uraian bahan
Field 28 sering berisi `4 BOX, WOODEN, NATURAL WOOD, ORDINARY,` - seluruhnya
masuk ke kotak Total Package, membuatnya meluber berikut koma menggantung.
Sekarang diambil jumlah + jenisnya saja: `4 BOX`.

### Yang diperiksa dan ternyata sudah benar
- **Kontainer FCL** akhirnya terbukti dengan data nyata: `CAIU4962745` +
  `FCL` dari field 27. Selama ini jalur itu belum pernah kena data berisi.
- **Satuan `NUMBER OF`** bukan kesalahan baca. Itu satuan CEISA
  `NUMBER OF BOX (BX)` yang tercetak terpotong dua baris; parser sudah
  menyambungnya dan mengambil kodenya, `BX`.

Total netto tiap dokumen dicocokkan dengan field 30: 6.478 · 3.800 · 18 ·
84 · 11,61 - semuanya sama persis.

---

## Revisi terbaru (putaran 20) - layar putih saat halaman dibuka

### Penyebabnya urutan, bukan kecepatan
Navbar dan SELURUH `<div>` halaman diberi kelas `d-none` di HTML, dan yang
melepasnya hanya `router()`. Tapi `router()` dipanggil **sesudah**
`await loadShipments()`. Akibatnya layar benar-benar kosong selama menunggu
jawaban database - bukan karena lambat, melainkan karena tidak ada apa pun
yang boleh tampil sebelum data datang.

Kerangka muat (skeleton) yang dibuat di putaran 9 pun tidak menolong sama
sekali: ia digambar ke dalam `#cardContainer`, yang saat itu masih ikut
tersembunyi. Jadi selama ini fitur itu tidak pernah benar-benar terlihat.

### Perbaikan
`router()` dipanggil LEBIH DULU, baru `await loadShipments()`. Navbar,
hero, kartu statistik, dan toolbar langsung tampil; area daftarnya diisi
kerangka muat. Keduanya berjalan dalam satu putaran tugas yang sama, jadi
peramban menggambarnya sekali - tidak ada kedipan layar kosong di antaranya.

Ditambah gaya dasar (warna latar, font) yang ditanam INLINE di halaman,
sehingga gambar pertama sudah memakai warna tema walau CSS dari CDN belum
tiba.

### Diverifikasi
Dengan database sengaja dilambatkan 1,5 detik: pada 350 ms navbar dan
halaman daftar SUDAH tampil dengan 4.524 karakter teks terbaca, dan
kerangka muat menggambar 3 kartu dengan ukuran nyata. Sebelumnya: nol.
